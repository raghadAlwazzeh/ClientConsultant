...
document.querySelectorAll('.nav-link').forEach(link => {
  ...
  // استرجاع الخيار المحدد من localStorage عند تحميل الصفحة
window.onload = function() {
  ...
  // إضافة حدث عند النقر على الرابط لتحديده وتخزينه في localStorage
  document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', function(event) {
      // إزالة الكلاس .selected من جميع العناصر
      document.querySelectorAll('.nav-link').forEach(item => item.classList.remove('selected'));
      
      // إضافة الكلاس .selected على العنصر الذي تم اختياره
      this.classList.add('selected');
  
      // تخزين الرابط المحدد في localStorage
...
//year option select
    ...
    populateYears("birth-date");
    populateYears("reise-date");
    populateYears("school_graduation_year");
    populateYears("training_completion_year");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    rows.forEach(row => {
        row.addEventListener("click", function () {
            ...
            if (url) {
                window.location.href = url; // انتقل إلى الرابط عند النقر
            }
        });
    });
});
